...
Mistral, Groq, NVIDIA NIM, OpenRouter and DeepSeek use the shared
OpenAI-compatible provider adapter. The router tries the selected primary
provider first, then any other configured providers, and falls back to the
deterministic council if all providers are unavailable.
